...
@Component
public class ConsumerGroupRunner implements ApplicationRunner {
    ...
    private static final Logger log = LoggerFactory.getLogger(ConsumerGroupRunner.class);

    private final KubeMQTemplate template;

    public ConsumerGroupRunner(KubeMQTemplate template) {
    ...
    public void run(ApplicationArguments args) {
        try {
            ...
            for (int i = 1; i <= 6; i++) {
                template.sendEvent("spring-events.consumer-group", "Message #" + i);
                log.info("Sent event #{} to spring-events.consumer-group", i);
            }
            Thread.sleep(POST_SEND_DRAIN_MS);
            log.info("Consumer group example completed.");
